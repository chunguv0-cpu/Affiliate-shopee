...
function buildInnerScript(endpoint: string, secret: string, productId: string): string {
  ...
  return (
    ...
    "var imgs=[];var seen={};u.forEach(function(x){if(!x)return;x=x.indexOf('//')===0?'https:'+x:x;if(/susercontent.com|cf.shopee.vn/i.test(x)&&/\\/file\\//i.test(x)&&!/(logo|favicon|sprite|placeholder|default|avatar|banner|qr[-_]?code|app[-_]?icon|appstore|googleplay|deo\\.shopeemobile|\\/web\\/|icon[-_.]|\\.svg|shopee[-_]?bag|tracking|pixel|1x1)/i.test(x)&&!seen[x]){seen[x]=1;imgs.push(x);}});" +
